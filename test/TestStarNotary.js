...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.symbol.call(), 'MSD');
});
...
it('lets a user transfer a star', async() => {
    ...
    let tokenId = 9;
    let instance = await StarNotary.deployed();
    let toUser = accounts[5];
    await instance.createStar('Awesome Star 3', tokenId, {from: accounts[6]});
    // 2. use the transferStar function implemented in the Smart Contract
    await instance.transferStar(toUser, tokenId, {from: accounts[6]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 10;
    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star 7', tokenId, {from: accounts[7]});
    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(tokenId);
    // 3. Verify if you Star name is the same
    assert.equal(starName, 'Awesome Star 7');
});